...
fn parse_ed25519_public_key(arr: &[u8]) -> Result<Ed25519PublicKey> {
    let bytes: [u8; ED25519_PUBLIC_KEY_LEN] = arr
        .try_into()
        .map_err(|_| Error::KeyError(EdCurve::Ed25519.name()))?;

    Ed25519PublicKey::from_compressed_bytes(bytes)
        .map_err(|_| Error::KeyError(EdCurve::Ed25519.name()))
}

fn parse_ed25519_secret_key(arr: &[u8]) -> Result<Ed25519SecretKey> {
    let bytes: [u8; ED25519_SECRET_KEY_LEN] = arr
        .try_into()
        .map_err(|_| Error::KeyError(EdCurve::Ed25519.name()))?;
