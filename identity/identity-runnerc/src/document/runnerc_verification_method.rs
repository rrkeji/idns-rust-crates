...
pub struct RunnercVerificationMethod(VerificationMethod);

impl RunnercVerificationMethod {
  /// Creates a new Merkle Key Collection Method from the given key collection.
  pub fn create_merkle_key<D>(did: RunnercDID, keys: &KeyCollection, fragment: &str) -> Result<Self>
  where
    D: MerkleDigest,
  {
    let tag: String = format!("#{}", fragment);
    let key: RunnercDIDUrl = did.to_url().join(tag)?;
  ...
  /// Creates a new [`RunnercVerificationMethod`] from the given `keypair`.
  ///
  /// WARNING: this derives a new DID from the keypair, which will not match the DID of a document
  /// created with a different keypair. Use [`RunnercVerificationMethod::from_did`] instead.
  pub fn from_keypair(keypair: &KeyPair, fragment: &str) -> Result<Self> {
    let key: &[u8] = keypair.public().as_ref();
    let did: RunnercDID = RunnercDID::new(key)?;

    Self::from_did(did, keypair.type_(), keypair.public(), fragment)
  }

  /// Creates a new [`RunnercVerificationMethod`] from the given `keypair` on the specified
  /// `network`.
  pub fn from_keypair_with_network(keypair: &KeyPair, fragment: &str, network: NetworkName) -> Result<Self> {
    let key: &[u8] = keypair.public().as_ref();
    let did: RunnercDID = RunnercDID::new_with_network(key, network)?;

    Self::from_did(did, keypair.type_(), keypair.public(), fragment)
  }

  /// Creates a new [`RunnercVerificationMethod`] from the given `did` and `keypair`.
  pub fn from_did(did: RunnercDID, key_type: KeyType, public_key: &PublicKey, fragment: &str) -> Result<Self> {
    let tag: String = format!("#{}", fragment);
    let key: RunnercDIDUrl = did.to_url().join(tag)?;
  ...
  pub fn try_from_mut(method: &mut VerificationMethod) -> Result<&mut Self> {
    ...
    //         RunnercVerificationMethod is transparent.
    Ok(unsafe { &mut *(method as *mut VerificationMethod as *mut RunnercVerificationMethod) })
  }
  ...
  pub unsafe fn new_unchecked_ref(method: &VerificationMethod) -> &Self {
    ...
    &*(method as *const VerificationMethod as *const RunnercVerificationMethod)
  }
  ...
  pub fn check_validity<T>(method: &VerificationMethod<T>) -> Result<()> {
    ...
    RunnercDID::check_validity(method.id().did())?;
    RunnercDID::check_validity(method.controller())?;
  ...
  pub fn id(&self) -> RunnercDIDUrl {
    // We ensure the validity of the id on creation.
    let did_url: &CoreDIDUrl = self.0.id();
    RunnercDIDUrl::try_from(did_url.clone()).expect("invalid RunnercDIDUrl")
    ...
    // unsafe { &*(did_url as *const CoreDIDUrl as *const RunnercDIDUrl) }
  }
  ...
  pub fn controller(&self) -> &RunnercDID {
    // SAFETY: We don't create methods with invalid DIDs and the layout of RunnercDID is transparent.
    unsafe { RunnercDID::new_unchecked_ref(self.0.controller()) }
  }

  /// Sets the method `controller` property.
  pub fn set_controller(&mut self, did: RunnercDID) {
...
impl Display for RunnercVerificationMethod {
  fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
    Display::fmt(&self.0, f)
  }
}

impl Debug for RunnercVerificationMethod {
  fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
    Debug::fmt(&self.0, f)
  }
}

impl Deref for RunnercVerificationMethod {
...
impl From<RunnercVerificationMethod> for VerificationMethod {
  fn from(other: RunnercVerificationMethod) -> Self {
    other.0
  }
}

impl From<RunnercVerificationMethod> for MethodRef {
  fn from(other: RunnercVerificationMethod) -> Self {
    other.0.into()
  }
}

impl TryFrom<VerificationMethod> for RunnercVerificationMethod {
  type Error = Error;
...
impl Revocation for RunnercVerificationMethod {
