...
impl PublishType {
  ...
  pub fn new(old_doc: &RunnercDocument, new_doc: &RunnercDocument) -> Option<PublishType> {
    if old_doc == new_doc {
      return None;
    }
  ...
  fn extract_signing_keys(document: &RunnercDocument) -> Vec<Option<&VerificationMethod>> {
    document
      .as_document()
...
mod test {
  ...
  fn document() -> RunnercDocument {
    let initial_keypair: KeyPair = KeyPair::new_ed25519().unwrap();
    let method: RunnercVerificationMethod = RunnercVerificationMethod::from_keypair(&initial_keypair, "embedded").unwrap();

    let mut old_doc: RunnercDocument = RunnercDocument::from_verification_method(method).unwrap();

    let keypair: KeyPair = KeyPair::new_ed25519().unwrap();
    let method2: RunnercVerificationMethod =
      RunnercVerificationMethod::from_did(old_doc.did().to_owned(), keypair.type_(), keypair.public(), "generic").unwrap();
  ...
  fn test_publish_type_insert_new_embedded_capability_invocation_method() -> Result<()> {
    ...
    let method2: RunnercVerificationMethod =
      RunnercVerificationMethod::from_did(old_doc.did().to_owned(), keypair.type_(), keypair.public(), "test-2")?;
  ...
  fn test_publish_type_update_key_material_of_existing_embedded_method() -> Result<()> {
    ...
    let keypair: KeyPair = KeyPair::new_ed25519()?;
    let verif_method2: RunnercVerificationMethod =
      RunnercVerificationMethod::from_did(new_doc.did().to_owned(), keypair.type_(), keypair.public(), "embedded")?;
  ...
  fn test_publish_type_update_key_material_of_existing_generic_method() -> Result<()> {
    ...
    let keypair: KeyPair = KeyPair::new_ed25519()?;
    let method_updated: RunnercVerificationMethod =
      RunnercVerificationMethod::from_did(new_doc.did().to_owned(), keypair.type_(), keypair.public(), "generic")?;
  ...
  fn test_publish_type_add_non_capability_invocation_method() -> Result<()> {
    ...
    let keypair: KeyPair = KeyPair::new_ed25519()?;
    let verif_method2: RunnercVerificationMethod =
      RunnercVerificationMethod::from_did(new_doc.did().to_owned(), keypair.type_(), keypair.public(), "test-2")?;
  ...
  fn test_publish_type_update_method_with_non_update_method_type() -> Result<()> {
    ...
    let method: RunnercVerificationMethod =
      RunnercVerificationMethod::create_merkle_key::<Sha256>(new_doc.did().to_owned(), &collection, "merkle")?;
  ...
  fn test_publish_type_update_method_with_non_update_method_type2() -> Result<()> {
    ...
    let collection = KeyCollection::new_ed25519(8)?;
    let method: RunnercVerificationMethod =
      RunnercVerificationMethod::create_merkle_key::<Sha256>(old_doc.did().to_owned(), &collection, "merkle")?;
    ...
    let method_new: RunnercVerificationMethod =
      RunnercVerificationMethod::create_merkle_key::<Sha256>(new_doc.did().to_owned(), &new_collection, "merkle")?;
